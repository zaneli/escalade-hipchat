...
object RateLimit extends DataHandler {
  def apply(map: Map[String, String]): RateLimit = {
    val limit = map.get("X-RateLimit-Limit").map(_.toInt).get
    val remaining = map.get("X-RateLimit-Remaining").map(_.toInt).get
    val reset = map.get("X-RateLimit-Reset").flatMap(x => sec2DateTime(x.toLong)).get
